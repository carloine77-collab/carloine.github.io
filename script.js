    ...
    function handleNewMomentSubmit() {
        ...
        if (content) {
            ...
            setTimeout(async () => {
                ...
                // --- V V V 核心修正就在这里 V V V ---
                // 只有当AI的回复有效，且不包含“错误”字样时，才执行后续操作
                if (aiCommentText && !aiCommentText.includes("错误")) {
                    ...
                    // 如果包含“错误”，我们就在控制台打印一条信息，但不会在朋友圈显示
                    console.log("AI response contained an error or was empty. Comment will not be posted.");
                }
                // --- ^ ^ ^ 修正结束 ^ ^ ^ ---
